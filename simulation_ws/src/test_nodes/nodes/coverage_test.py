...
class Map(object):
    ...
    def init_coverage_map(self):
        self.initial_map = Image.fromarray(self.map.astype(np.uint8))
        self.coverage_map = self.initial_map.copy()
        self.initial_free_cells = len(
            [px for px in list(self.initial_map.getdata()) if px == self.IMAGE_FREE_AREA])
    ...
    def add_coverage(self, robot_pose, footprint):
        ...
        if self.last_robot_poses is not None:
            # Draw from last_robot_poses to robot_pose_index
            pass
        self.last_robot_poses = robot_pose
        self.robot_poses.append(robot_pose_index)
    ...
    def get_path_image(self):
        path_image = self.initial_map.copy()
        path_image = path_image.convert('RGB')
        draw = ImageDraw.Draw(path_image)
        draw.line(self.robot_poses, fill=self.PATH_COLOR, width=2)
        return path_image
...
class CoverageTest(unittest.TestCase):
    ...
    def odom_cb(self, msg):
        if not self.is_cancelled:
            ...
            if self.map.get_coverage() >= self.coverage_threshold:
    ...
    def cancel_job(self):
        ...
        if self.map_image_path:
            self.map.get_path_image().save(self.map_image_path)
        # TODO: REMOVE
    ...
    def get_params(self):
        ...
        """
        self.map_image_path = rospy.get_param("image_path")
